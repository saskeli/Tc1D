...
annealModelRec modKetchamEtAl = {-19.844,  0.38951, -51.253, -7.6423,
                                 -0.12327, -11.988, 0.0};

annealModelRec modTILm = {-1.66965, 0.0000241755, -12.4864, 0.000843004,
                          0.675508, 4.16615,      0.0};
annealModelRec modTILc = {-2.36910, 0.0000603834, -8.65794, 0.000972676,
                          0.404700, 1.65355,      9.0};
/* This is Crowley Durango model from Willett, 1992, in turn from where???
  annealModelRec modCrowDur =
  {-2.959,0.00008336,-17.0585,0.0005382,0.51,2.97,0.0};
*/
annealModelRec modCrowDur = {-3.202, 0.00009367, -19.6328, 0.0004200,
                             0.49,   3.00,       0.0};
annealModelRec modCrowFAp = {-1.508, 0.00002076, -10.3227, 0.0009967,
                             0.76,   4.30,       0.0};
annealModelRec modCrowSrAp = {-1.123, 0.00001055, -5.0085, 0.001195,
                              0.97,   4.16,       0.0};
...
l0Def l0vsKparDefault[L0_DEF_SIZE] = {
    {0.283, 15.63},     /* dpar mean */
...
table meanToCpar[NUM_RMEAN_TO_RCPAR] = {
    {1.0, 1.0},           {0.95881, 0.970588},  {0.917508, 0.941176},
    {0.876082, 0.911765}, {0.834515, 0.882353}, {0.792789, 0.852941},
    {0.750883, 0.823529}, {0.708769, 0.794118}, {0.666416, 0.764706},
    {0.644024, 0.735294}, {0.592261, 0.705882}, {0.543816, 0.676471},
    {0.499281, 0.647059}, {0.460204, 0.617647}, {0.428183, 0.588235},
    {0.403557, 0.558824}, {0.385398, 0.529412}, {0.0, 0.0}};
#define NUM_RMEAN_TO_RDEN 18
table lengthToDensity[NUM_RMEAN_TO_RDEN] = {
    {1.0, 1.0},           {0.95881, 0.956529},  {0.917508, 0.912921},
    {0.876082, 0.869161}, {0.834515, 0.82523},  {0.792789, 0.781104},
    {0.750883, 0.736758}, {0.708769, 0.692159}, {0.666416, 0.647269},
    {0.644024, 0.541049}, {0.592261, 0.442989}, {0.543816, 0.358767},
    {0.499281, 0.286844}, {0.460204, 0.225362}, {0.428183, 0.172461},
    {0.403557, 0.126281}, {0.385398, 0.085095}, {0.0, 0.0}};
 ...
 */
double InitialTrackLength(double kineticPar, int kineticParType, int doProject,
                          double l0user) {
  int index;
 ...
 */
double FindValue(double par, tablePtr target_table, int numTable) {
  int i;
  double frac;

  if (par >= target_table[0].par) return (target_table[0].value);
  if (par <= target_table[numTable - 1].par) return (target_table[numTable - 1].value);
  for (i = 1; par < target_table[i].par; i++);
  frac = (par - target_table[i].par) / (target_table[i - 1].par - target_table[i].par);
  return (target_table[i - 1].value * frac + target_table[i].value * (1.0 - frac));
}
...
int InterpolateTTPathKet(int numTTDefs, ttPathPtr tTDef, int* numTTNodes,
                         ttPathPtr tTPath, double pctPerTimeStep) {
  int dN, n;
  ...
  double currDefTimeStep; /* Default time step for the current path segment
                             (m.y.) */
  ...
  *numTTNodes = 1;
  ...
  for (dN = numTTDefs - 1; dN > 0; dN--) {
    ...
    rate = (tTDef[dN].temp - tTDef[dN - 1].temp) /
           (tTDef[dN].time - tTDef[dN - 1].time + 0.0001);
    absRate = fabs(rate);
    tempPerTimeStep = absRate * defTimeStep;
    currDefTimeStep = (tempPerTimeStep <= MAX_TEMP_STEP_KET)
                          ? defTimeStep
                          : MAX_TEMP_STEP_KET / absRate;
    maxTMult = rate > 0 ? 0 : -1;
    ...
    while (tTPath[*numTTNodes - 1].time > tTDef[dN - 1].time) {
      ...
      /* Check to see if this is final step for this segment.  A small factor
        is added to account for the possibility of roundoff. NOTE: This factor
        must be significantly shorter than any time step. */
      if (timeStep + 0.001 >
          tTPath[*numTTNodes - 1].time - tTDef[dN - 1].time) {
        ...
        tTPath[*numTTNodes].temp =
            tTPath[*numTTNodes - 1].temp - rate * timeStep;
        /*        printf("TR %d %f %f %f %f
         * \n",*numTTNodes,tTPath[*numTTNodes].temp,rate,timeStep,tTPath[*numTTNodes].time);
         */
      }
  ...
  for (n = 0; n < *numTTNodes; n++) tTPath[n].time *= SECS_PER_MA;
...
int InterpolateTTPath(int numTTDefs, ttPathPtr tTDef, int* numTTNodes,
                      ttPathPtr tTPath, double pctPerTimeStep) {
  int dN, n;
  double rate, absRate;     /* Rate of temperature change (K/m.y.) */
  double maxTMult;          /* Used to help find max temp in a time step */
  ...
  *numTTNodes = 1;
  ...
  for (dN = numTTDefs - 1; dN > 0; dN--) {
    ...
    if (absRate < 0.1)
      ...
      /*     altTimeStep = (absRate > 0.1) ? MAX_TEMP_STEP/absRate :
       * defTimeStep;      */
    }
    while (tTPath[*numTTNodes - 1].time > tTDef[dN - 1].time) {
      /* Make sure we haven't specified too many nodes */
 ...
 */
void ChooseAnnealingModel(int annealingModel, int kinParType, int* numConv,
                          annealModelRec* annMod, annealConvPtr* annConv) {
...
void CalcModelLengthsKet(ttPathPtr tTPath, int numTTNodes, double redLength[],
  ...
  switch (kinParType) {
    case ETCH_PIT_LENGTH:
      if (kinPar <= 1.75)
        ...
        rmr0 = 1.0 - exp(0.647 * (kinPar - 1.75) - 1.834);
      break;
    case CL_WT_PCT:
  ...
  /*/ equivTotAnnLen is the length of the more resistant apatite at the length
  of
  //  total annealing for the less resistant apatite we're modeling.
  //  In the future, if this routine is adapted to solve for many different
  apatite
  //  kinetic populations at once, we would use the rmr0 and k values for the
  most
  ...
  */
  ...
  for (node = numTTNodes - 2; node >= 0; node--) {
    ...
    if (redLength[node] < 0.999) {
      ...
      equivTime =
          pow((1.0 - pow(redLength[node], modKetchamEtAl.b)) / modKetchamEtAl.b,
              modKetchamEtAl.a);
      equivTime = ((equivTime - 1.0) / modKetchamEtAl.a - modKetchamEtAl.c0) /
                  modKetchamEtAl.c1;
      equivTime =
          exp(equivTime * (tempCalc - modKetchamEtAl.c3) + modKetchamEtAl.c2);
    }
  }
}
...
void CalcModelLengths(ttPathPtr tTPath, int numTTNodes, double redLength[],
                      double kinPar, annealModelRec annMod,
                      annealConvPtr annConv, int numConv, int* firstTTNode) {
  int node, nodeB;
  ...
  if (numConv) {
    if (kinPar >= annConv[0].kPar)
      c0 = -1;
    else
      for (c0 = numConv - 1; c0 && (kinPar > annConv[c0].kPar); c0--);
    if (c0 == -1) {
      a0 = annConv[0].a0;
      a1 = annConv[0].a1;
      ...
      frac = (kinPar - annConv[c0 + 1].kPar) /
             (annConv[c0].kPar - annConv[c0 + 1].kPar);
  ...
  for (node = numTTNodes - 2; node >= 0; node--) {
    ...
    if (redLength[node] < totAnnealLen) redLength[node] = 0.0;
    ...
    if (redLength[node] < 0.999) {
      equivTime =
          pow((1.0 - pow(redLength[node], annMod.b)) / annMod.b, annMod.a);
      equivTime = ((equivTime - 1.0) / annMod.a - annMod.c0) / annMod.c1;
      equivTime =
          exp(equivTime * (1.0 / meanTemps[node - 1] - annMod.c3) + annMod.c2);
    }
  }
}

/*  AgeCorrectionKet
    Does the conversion from length to density for the Ketcham et al., 1999
   model. The routine is placed "way up here" because it will also be used to
   estimate bias for population summing.
...
void SumPopulationsKet(int numPDFPts, int numTTNodes, int firstTTNode,
                       int doProject, int usedCf, ttPathPtr tTPath,
                       double pdfAxis[], double pdf[], double cdf[],
                       double initLength, double redLength[]) {
  int i, j;
  ...
  for (j = firstTTNode; j < numTTNodes - 1; j++) {
    ...
    weight = wt1 - wt2;
    wt1 = wt2;
    rmLen = usedCf ? 1.396 * redLength[j] - 0.4017
                   : -1.499 * redLength[j] * redLength[j] +
                         4.150 * redLength[j] - 1.656;
  ...
  for (i = 1; i < numPDFPts; i++)
    cdf[i] = cdf[i - 1] +
             ((pdf[i] + pdf[i - 1]) / 2.0) * (pdfAxis[i] - pdfAxis[i - 1]);
  /* Normalize */
...
/*  SumPopulations
    ...
    lengths and finding the standard deviation of the population distribution.
 */
void SumPopulations(int numPDFPts, int numTTNodes, int firstTTNode,
                    int doProject, ttPathPtr tTPath, double pdfAxis[],
                    double pdf[], double cdf[], double initLength,
                    double redLength[]) {
  int i, j;
  ...
  for (j = firstTTNode; j < numTTNodes - 1; j++) {
    ...
    weight = wt1 - wt2;
    wt1 = wt2;
    rLen = doProject ? FindValue(redLength[j], meanToCpar, NUM_RMEAN_TO_RCPAR)
                     : redLength[j];
...
/*  CalcModelStats
    ...
    distribution.
 */
void CalcModelStats(int numPopulations, int numPDFPts, double pdfAxis[],
                    double pdf[], statsPtr stats) {
  ...
  for (i = 0; i < numPDFPts; i++) {
    ...
    stats->skewness +=
        weight * pow((pdfAxis[i] - stats->mean) / stats->stDev, 3.0);
    stats->kurtosis +=
        weight * pow((pdfAxis[i] - stats->mean) / stats->stDev, 4.0);
  }
 ...
 */
void CalcModelAgesKet(ttPathPtr tTPath, double redLength[], int numTTNodes,
                      int firstNode, double* oldestModelAge, double* ftModelAge,
  ...
  *oldestModelAge = tTPath[firstNode].time / SECS_PER_MA;
  for (*ftModelAge = 0.0, node = firstNode; node < numTTNodes - 2; node++) {
    ...
    *ftModelAge += AgeCorrectionKet(midLength) *
                   (tTPath[node].time - tTPath[node + 1].time);
  }
  *ftModelAge += AgeCorrectionKet(redLength[numTTNodes - 2]) *
                 (tTPath[node].time - tTPath[node + 1].time);
 ...
 */
void CalcModelAges(ttPathPtr tTPath, double redLength[], int numTTNodes,
                   int firstNode, double* oldestModelAge, double* ftModelAge,
                   double stdLengthReduction) {
  int node;
  double midLength;
  ...
  *oldestModelAge = tTPath[firstNode].time / SECS_PER_MA;
  for (*ftModelAge = 0.0, node = firstNode; node < numTTNodes - 2; node++) {
    ...
    midLength = (redLength[node] + redLength[node + 1]) / 2.0;
    *ftModelAge +=
        AgeCorrection(midLength) * (tTPath[node].time - tTPath[node + 1].time);
  }
  *ftModelAge += AgeCorrection(redLength[numTTNodes - 2]) *
                 (tTPath[node].time - tTPath[node + 1].time);
  /* Account for length reduction in length standard, convert to Ma */
  *ftModelAge /= (stdLengthReduction * SECS_PER_MA);
}
...
/*  ForwardModel
    ...
    fission-track age.
 */
int ForwardModel(int numTTDefs, double stdLengthReduction, double kinPar,
                 double pctPerTimeStep, int annealModel, int doProject,
                 int usedCf, int kinParType, double l0user, double cdf[],
                 int numPDFPts, double pdfAxis[], double pdf[],
                 double* oldestModelAge, double* ftModelAge,
  ...
  *numPopulations = 0;
  ...
  if (annealModel == KETCHAM_ET_AL) {
    if (InterpolateTTPathKet(numTTDefs, tTDef, &numTTNodes, tTPath,
                             pctPerTimeStep)) {
      CalcModelLengthsKet(tTPath, numTTNodes, redLength, kinPar, kinParType,
                          &firstTTNode);
      SumPopulationsKet(numPDFPts, numTTNodes, firstTTNode, doProject, usedCf,
    ...
    if (InterpolateTTPath(numTTDefs, tTDef, &numTTNodes, tTPath,
                          pctPerTimeStep)) {
      ChooseAnnealingModel(annealModel, kinParType, &numConv, &annMod,
                           &annConv);

      CalcModelLengths(tTPath, numTTNodes, redLength, kinPar, annMod, annConv,
                       numConv, &firstTTNode);
      SumPopulations(numPDFPts, numTTNodes, firstTTNode, doProject, tTPath,
...
void ketch_main(int* ntime, float ketchtime[], float ketchtemp[], double* alo,
                double* final_age, double* oldest_age, double* fmean,
                double fdist[]) {
...
#define nbins 200
  ...
  for (i = 0; i < numPDFPts; i++)
    pdfAxis[i] = (double)(i * 1.0 + 0.5) * 20.0 / numPDFPts;
  ...
  ktime = *ntime;
  for (i = 0; i < *ntime; i++) {
    kerryTt[i].time = ketchtime[i];  //+0.001*i; /*ad hoc modification to ensure
                                     //times are different....) */
  ...
  ForwardModel(ktime, lengthred, kinetic_par, (double)1.0, annealmodel,
               doProject, usedCf, kinpar, *alo, cdf, numPDFPts, pdfAxis, pdf,
