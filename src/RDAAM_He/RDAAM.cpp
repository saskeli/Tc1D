...
double *aDepl238, *aDepl235, *aDepl232,
    *aDepl147;  // Alpha depletion including zoning (% retained)
double *aEjOnly238, *aEjOnly235, *aEjOnly232,
    *aEjOnly147;  // Alpha ejection only
double *nmpg238, *nmpg235, *nmpg232,
    *nmpg147;  // Compositional profiles (nanomoles per gram)
...
double* betaTrap;  // Represents radially varying diffusivity in trapping model
                   // (cm^2/s)
double *difft_n,
    *difft_np1;  // Radially varying diffusivity at current and next time step.
...
int endNode;  // Earliest t-T node to calculate on (when damage starts being
              // retained)
...
void GeneralInit(int newPrecision, double grainRadius, double ppm_U,
                 double ppm_Th, double ppm_Sm) {
...
void RDAAM_Init(int newPrecision, double grainRadius, double ppm_U,
                double ppm_Th, double ppm_Sm) {
  GeneralInit(newPrecision, grainRadius, ppm_U, ppm_Th, ppm_Sm);
...
void ZRDAAM_Init(int newPrecision, double grainRadius, double ppm_U,
                 double ppm_Th, double ppm_Sm) {
  GeneralInit(newPrecision, grainRadius, ppm_U, ppm_Th, ppm_Sm);
...
void RDAAM_PrepModel() {
  ...
  for (int i = 0; i < rdim; i++) {
    ...
    // Calculate alpha ejection-only array, which we'll need in a variety of
    // cases
    xi = (i + 0.5) * gridSpacing * 1.e4;  // distance from center, in microns
    if (xi > (radius - asDist.asU238)) {
      xs = (xi * xi + radius * radius - asDist.asU238 * asDist.asU238) /
           (2. * xi);
    ...
    if (xi > (radius - asDist.asU235)) {
      xs = (xi * xi + radius * radius - asDist.asU235 * asDist.asU235) /
           (2. * xi);
    ...
    if (xi > (radius - asDist.asTh232)) {
      xs = (xi * xi + radius * radius - asDist.asTh232 * asDist.asTh232) /
           (2. * xi);
    ...
    if (xi > (radius - asDist.asSm147)) {
      xs = (xi * xi + radius * radius - asDist.asSm147 * asDist.asSm147) /
           (2. * xi);
...
void RDAAM_CalcAlphaCorrectionFactor() {
  ...
  for (int i = 0; i < rdim; i++) {
    ...
    dTotal += vol * (aDepl238[i] * nmpg238[i] * rate238 +
                     aDepl235[i] * nmpg235[i] * rate235 +
                     aDepl232[i] * nmpg232[i] * rate232 +
                     aDepl147[i] * nmpg147[i] * rate147);
    ndTotal += vol * (nmpg238[i] * rate238 + nmpg235[i] * rate235 +
                      nmpg232[i] * rate232 + nmpg147[i] * rate147);
 ...
 */
int RDAAM_InterpolateTTPath(double startTime) {
  int dN, n;
  TTPathPoint nextPt;
  ...
  if ((startTime == 0.0) || (startTime > nextPt.time))
    startTime = nextPt.time;
  else
    truncateStart = true;
  ...
  for (dN = tTDef->size() - 1; dN > 0; dN--) {
    /* Calculate rate for this t-T segment: at least 5 steps (probably not
     * required) */
    double currMaxTimeStep =
        (tTDef->at(dN).time - tTDef->at(dN - 1).time) * 0.2;
    if (currMaxTimeStep == 0.0) return (0);  // Bad time step

    rate = (tTDef->at(dN).temperature - tTDef->at(dN - 1).temperature) /
           (tTDef->at(dN).time - tTDef->at(dN - 1).time);
    absRate = fabs(rate);
    tempPerTimeStep = absRate * defTimeStep;
    currDefTimeStep =
        (tempPerTimeStep <= maxTempStep) ? defTimeStep : maxTempStep / absRate;
...
void RDAAM_ExtractHeProfile() {
  ...
  for (i = 0; i < rdim; i++) {
    ...
    gam[i] =
        heProfile[i] * 4.0 * PI * rad * rad;  // Convert to integration quantity
  }
  ...
  // Fill in crystal center (a half-node spacing before first node); NR
  // formula 4.1.10
  heTot += 0.5 * gridSpacing *
           (gam[0] * 55. / 24. - gam[1] * 59. / 24. + gam[2] * 37. / 24. -
            gam[3] * 9. / 24.);
...
void RDAAM_InitFTAnnealingTraps(double**& annealingTraps, bool optimize) {
  ...
  double equivTotAnnealLen =
      pow(trapTotAnnealLen, invTrapKappa) * (1.0 - trapRmr0) + trapRmr0;
  annealParamRec zirAnnealParams = {
      6.24354, -0.11977, -314.93688, -14.286838, -0.057206897, 0.0, 0.0};
  annealParamRec apAnnealParams = {0.39528, 0.01073, -65.12969, -7.91715,
                                   0.04672, 0.0,     0.0};  // K07
  ...
  double R = isZircon
                 ? 0.000552
                 : 0.000815;  // Etchable range of one fission fragment (in cm)
  double densityConv = isZircon ? 6.022e14
                                : N_PER_NMOL * DENSITY_APATITE *
                                      (U238YR_SF / U238YR) * etaQ * R /
                                      8.0;  // Density conversion factor

  int tsNode1, initEndpoint;

  endNode = 0;
  if (optimize) {
    // In the first step, find age of the "oldest trap" persisting at present
    // day.
    fInit = dvector(0, numTTNodes - 1);
    equivTime = 0;
    tempCalc = log(2.0 / (tTPath[numTTNodes - 2].temperature +
                          tTPath[numTTNodes - 1].temperature));
    for (node = numTTNodes - 2; node >= 0; node--) {
      ...
      if (node == 0)
        ...
        tempCalc = log(
            2.0 / (tTPath[node - 1].temperature + tTPath[node].temperature));
        equivTime = exp(x1 * (tempCalc - annealParams.c3) + annealParams.c2);
      }
    }
    // Next, find age of the oldest trap when the present-day oldest trap
    // formed. This ensures that we start early enough that all traps are
    // represented.
    if (otNode > 0) {
      equivTime = 0;
      for (node = otNode; node >= 0; node--) {
        timeInt = tTPath[node].time - tTPath[node + 1].time + equivTime;
        if (timeInt <= 0.0)
          continue;  // For the occasional zero-length time step
        x1 = (log(timeInt) - annealParams.c2) / (tempCalc - annealParams.c3);
        x2 = annealParams.c0 + annealParams.c1 * x1;
        fInit[node] = pow(x2, 1.0 / annealParams.a) + 1.0;
        fInit[node] = (fInit[node] <= 0) ? 0.0 : 1.0 / fInit[node];
    ...
    tsNode1 = numTTNodes - 2;
  // Now create fstate array, holding f of traps formed in each previous time
  // step
  ...
  if (optimize) {
    if (psiUnits == PSI_CMSQ_TRACK) {
      for (node = numTTNodes - 2; node >= initEndpoint; node--) {
        fState[numTTNodes - 2][node] =
            pow((fInit[node] - trapRmr0) / (1.0 - trapRmr0), trapKappa);
        if (fState[numTTNodes - 2][node] > 0.757)
          fState[numTTNodes - 2][node] =
              1.600 * fState[numTTNodes - 2][node] - 0.599;
        else
          fState[numTTNodes - 2][node] =
              (9.205 * fState[numTTNodes - 2][node] *
                   fState[numTTNodes - 2][node] -
               9.157 * fState[numTTNodes - 2][node] + 2.269);
      }
    } else if (isZircon) {
      for (node = numTTNodes - 2; node >= initEndpoint; node--) {
        fState[numTTNodes - 2][node] = fInit[node];
        fState[numTTNodes - 2][node] =
            1.25 * fState[numTTNodes - 2][node] - 0.25;
      }
    }
    //
  ...
  for (tsNode = tsNode1; tsNode >= endNode; tsNode--) {
    equivTime = 0;
    tempCalc = log(
        2.0 / (tTPath[tsNode].temperature + tTPath[tsNode + 1].temperature));
    for (node = tsNode; node >= endNode; node--) {
      ...
      fState[tsNode][node] =
          (fState[tsNode][node] <= 0) ? 0.0 : 1.0 / fState[tsNode][node];
    ...
    if (psiUnits == PSI_CMSQ_TRACK) {
      for (int tempNode = tsNode; tempNode >= node; tempNode--) {
        fState[tsNode][tempNode] =
            pow((fState[tsNode][tempNode] - trapRmr0) / (1.0 - trapRmr0),
                trapKappa);
        if (fState[tsNode][tempNode] > 0.757)
          fState[tsNode][tempNode] = 1.600 * fState[tsNode][tempNode] - 0.599;
        else
          fState[tsNode][tempNode] =
              (9.205 * fState[tsNode][tempNode] * fState[tsNode][tempNode] -
               9.157 * fState[tsNode][tempNode] + 2.269);
      }
    } else if (isZircon) {
      for (int tempNode = tsNode; tempNode >= node; tempNode--) {
  ...
  for (tsNode = numTTNodes - 2; tsNode >= endNode; tsNode--) {
    ...
    heU238 = 8. * (exp(U238SEC * tTPath[tsNode].time) -
                   exp(U238SEC * tTPath[tsNode + 1].time));
    heU235 = 7. * (exp(U235SEC * tTPath[tsNode].time) -
                   exp(U235SEC * tTPath[tsNode + 1].time));
    heTh232 = 6. * (exp(TH232SEC * tTPath[tsNode].time) -
                    exp(TH232SEC * tTPath[tsNode + 1].time));
    heSm147 = 1. * (exp(SM147SEC * tTPath[tsNode].time) -
                    exp(SM147SEC * tTPath[tsNode + 1].time));
    // Then combine for each node
    globalHeState = densityConv * (nmpg238[0] * heU238 + nmpg235[0] * heU235 +
                                   nmpg232[0] * heTh232 + nmpg147[0] * heSm147);
...
void RDAAM_CalcHeAge(bool optimize) {
  ...
  double** annealingTraps =
      NULL;  // 2D matrix to hold trap info if there's annealing
  //double invRadCubed;
  diff = 0;
  ...
  for (node = endNode; size_t(node) < tTPath.size() - 1; node++) {
    dt = tTPath[node].time - tTPath[node + 1].time;
    // Occasionally nodes will be spaced below floating-point resolution
    // limit...
    if (dt <= 0.0) continue;

    diff_n =
        dInf * exp(-E * 1000.0 / (UNIV_GAS_CONST * tTPath[node].temperature));
    diff_np1 = dInf * exp(-E * 1000.0 /
                          (UNIV_GAS_CONST * tTPath[node + 1].temperature));
    if (isZircon) {
      diffN17_n =
          psi * exp(-Et * 1000.0 / (UNIV_GAS_CONST * tTPath[node].temperature));
      diffN17_np1 = psi * exp(-Et * 1000.0 /
                              (UNIV_GAS_CONST * tTPath[node + 1].temperature));
    } else {
      trapExpTerm_n =
          exp(Et * 1000.0 / (UNIV_GAS_CONST * tTPath[node].temperature));
      trapExpTerm_np1 =
          exp(Et * 1000.0 / (UNIV_GAS_CONST * tTPath[node + 1].temperature));
    }
    ...
    for (i = 0; i < rdim; i++) {  // Main tridiagonal components
      ...
      if (isZircon) {
        if (node != endNode)
          ...
          fc = 1. - fa;
          difft_n[i] = (1. / (tortuosity * fc * fc * fc * radCmSq / diff +
                              fa * fa * fa * radCmSq / diffN17_n)) *
                       radCmSq;
        }
        // Next time step
        ...
        fc = 1. - fa;
        difft_np1[i] = (1. / (tortuosity * fc * fc * fc * radCmSq / diff_np1 +
                              fa * fa * fa * radCmSq / diffN17_np1)) *
                       radCmSq;
      } else {  // Flowers et al 2009
        trapDiffTerm = annealingTraps[node][i] * annealingTraps[node][i] *
                       annealingTraps[node][i];
        trapDiffTerm = (psi * annealingTraps[node][i] + polyA * trapDiffTerm) *
                           trapExpTerm_n +
                       1.0;
        difft_n[i] = diff_n / trapDiffTerm;
        trapDiffTerm = annealingTraps[node + 1][i] *
                       annealingTraps[node + 1][i] *
                       annealingTraps[node + 1][i];
        trapDiffTerm =
            (psi * annealingTraps[node + 1][i] + polyA * trapDiffTerm) *
                trapExpTerm_np1 +
            1.0;
    ...
    b[0] = (-preBeta + difft_n[1] + 2.0 * difft_n[0]) * u[0] -
           difft_n[1] * u[1] - prodHe[0];
    sup[0] = difft_np1[1];
    sub[0] = difft_np1[0];
    // Zero Dirichlet BC at right
    diag[rdim - 1] = -(preBeta + difft_np1[rdim - 1] + difft_np1[rdim]);
    b[rdim - 1] = (-preBeta + difft_n[rdim - 1] + difft_n[rdim]) * u[rdim - 1] -
                  difft_n[rdim - 1] * u[rdim - 2] - prodHe[rdim - 1];
    ...
    for (i = 1; i < rdim - 1; i++) {
      ...
      b[i] = (-preBeta + difft_n[i] + difft_n[i + 1]) * u[i] -
             difft_n[i + 1] * u[i + 1] - difft_n[i] * u[i - 1] - prodHe[i];
      sup[i] = difft_np1[i + 1];
      sub[i] = difft_np1[i];
    }
  ...
  while (hiAge - loAge > ageConv) {
    ...
    midVal = total238 * exp(U238YR * midAge) + total235 * exp(U235YR * midAge) +
  ...
  leftSum = total232 * ft232 + total235 * ft235 + total238 * ft238 +
            total147 * ft147 + totalHe;
  loAge = 0;
...
// RDAAM_FreeCalcArrays
// Use this to clean up memory after you're done with all of your RDAAM
// calculations
 ...
 */
void dpolint2(double xa[], double ya[], int n, double x, double* y,
              double* dy) {
  int i, m, ns = 1;
...
// It provides values for variables heModelAge and heCorrModelAge in Ma, and
// totHe in nmol/g, and overall returns 1 if successful, 0 if not. Set optimize
// to true for geological paths to make them calculate faster, and false for
// degassing (i.e. any history that has short, high-temperature steps at the
// end).
int RDAAM_Calculate(TTPath* tTInput, double& age, double& corrAge,
                    double& totHe, bool optimize) {
  age = 0.0;
  corrAge = 0.0;
  totHe = 0.0;
  if (!paramsOK) return (0);
  tTDef = tTInput;
  if (RDAAM_InterpolateTTPath(0.0)) {
